...
def Zebra():
    ...
    # Names = 'Englishman Spaniard Norwegian Ukranian Japanese'.split()
    # Smokes = 'Kools Chesterfields Winston LuckyStrike Parliaments'.split()
    Names = 'Albert Dietfried Virgiljus Bricius Gallchobhar'.split()
    Sports = 'rugby baseball debate soccer starcraft2'.split()
    Lawns = 'grass astroturf trees rocks flowers'.split()
    Transport = 'walk bicycle motorcycle bus car'.split()
    variables = Names + Sports + Lawns + Transport
    domains = {}
    for var in variables:
        domains[var] = list(range(1, 6))
    domains['debate'] = [2]
    domains['bicycle'] = [4]
    neighbors = parse_neighbors("""Albert: baseball; Dietfried: grass; Virgiljus: rugby; Gallchobhar: walk; Bricius: Virgiljus; astroturf: baseball; flowers: trees rocks; baseball: motorcycle; soccer: bus; trees: car; Gallchobhar: rugby""", variables)
    for type in [Names, Sports, Lawns, Transport]:
        for A in type:
            for B in type:
                if A != B:
    ...
    def zebra_constraint(A, a, B, b, recurse=0):
        ...
        if A == 'Albert' and B == 'baseball':
            return same
        if A == 'Dietfried' and B == 'grass':
            return same
        if A == 'Virgiljus' and B == 'rugby':
            return same
        if A == 'Gallchobhar' and B == 'walk':
            return same
        if A == 'Bricius' and B == 'Virgiljus':
            return next_to
        if A == 'astroturf' and B == 'baseball':
            return same
        if A == 'flowers' and B in ['trees', 'rocks']:
            return next_to
        ...
        if ((A in Transport and B in Transport) or
